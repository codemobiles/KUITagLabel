...
open class KUITagView: UIView {
    ...
    open func configure(_ tag: KUITag) {
        ...
        addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-\(tag.config.insets.left)-[view]-\(tag.config.insets.right)-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["view": contentView]))
        addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-\(tag.config.insets.top)-[view]-\(tag.config.insets.bottom)-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["view": contentView]))
        ...
        if let backgroundImage = tag.config.backgroundImage {
            ...
            contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-0-[view]-0-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["view": backgroundImgView]))
        }
        ...
        contentView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-\(tag.config.titleInsets.top)-[label]-\(tag.config.titleInsets.bottom)-|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["label": label]))
        ...
        if let style = NSParagraphStyle.default.mutableCopy() as? NSMutableParagraphStyle {
            ...
            let attr = NSAttributedString(string: tag.title,
                                          attributes: [
                                            ...
                                            NSAttributedString.Key.font: tag.config.titleFont,
                                            NSAttributedString.Key.foregroundColor: tag.config.titleColor])
